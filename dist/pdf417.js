...
exports.PDF417 = void 0;
const pdf417_clusters_1 = require("./pdf417_clusters"); // clusters.tsからCLUSTERSをインポート
class PDF417 {
    constructor(options = {}) {
        this.options = {
    ...
    generate(code) {
        ...
        const resolvedEcl = this.getErrorCorrectionLevel(this.options.errorCorrectionLevel, numcw);
        ...
        let cols = Math.round((Math.sqrt(4761 + 68 * this.options.aspectRatio * this.options.rowHeight * nce) -
            69) /
            34);
        // adjust cols
        if (cols < 1)
        ...
        if (size > 928) {
            ...
            if (Math.abs(this.options.aspectRatio - (17 * 29) / 32) <
        ...
        for (let r = 0; r < rows; ++r) {
            let rowPatternStr = "0".repeat(this.options.quietH) + PDF417.START_PATTERN; // row start code with quiet zone
            ...
            for (let c = 0; c < cols; ++c) {
                rowPatternStr += this.getCodewordPattern(currentClusterSet[codewords[k]]);
                ++k;
            }
            // right row indicator
            switch (cid) {
    ...
    getInputSequences(code) {
        ...
        const numseq = numseqMatches.map((match) => [
            match[1],
            match.index,
        ]);
        ...
        for (const seqItem of numseq) {
            ...
            if (currentNumSeqOffset > offset) {
                ...
                const textseqMatches = Array.from(prevseq.matchAll(/([\x09\x0a\x0d\x20-\x7e]{5,})/g));
                const textseq = textseqMatches.map((match) => [
                    match[1],
                    match.index,
                ]);
                ...
                for (const txtSeqItem of textseq) {
                    ...
                    if (currentTextSeqOffset > txtoffset) {
                        ...
                        const prevtxtseq = prevseq.substring(txtoffset, currentTextSeqOffset);
                        if (prevtxtseq.length > 0) {
                            // add BYTE sequence
                            if (prevtxtseq.length === 1 &&
    ...
    getCompaction(mode, code, addmode = true) {
        ...
        switch (mode) {
            case 900: {
                ...
                const txtarr = []; // array of characters and sub-mode switching characters
                        ...
                        for (let s = 0; s < 4; ++s) {
                            ...
                            if (s !== submode) {
                                ...
                                if (k !== -1) {
                                    ...
                                    const nextCharIsCurrentSubmode = i + 1 < codelen &&
                                        PDF417.TEXT_SUBMODES[submode].indexOf(code.charCodeAt(i + 1)) !== -1;
                                    ...
                                    if ((i + 1 === codelen || nextCharIsCurrentSubmode) &&
                                        (s === 3 || (s === 0 && submode === 1))) {
                                        // Shift
                                        if (s === 3)
                                            txtarr.push(29); // Shift to Punctuation
                                        else
                                            txtarr.push(27); // Shift from Lower to Alpha (tsl)
                                    }
                                    else {
                                        // Latch
    ...
    getErrorCorrectionLevel(ecl, numcw) {
        ...
        if (maxecl < 0)
            maxecl = 0; // Ensure it's at least 0
        let resolvedEcl = ecl;
        ...
        if (resolvedEcl < 0 || resolvedEcl > 8) {
            if (numcw < 41)
                ...
                resolvedEcl = maxecl; // Use calculated max if numcw is very high
        }
    ...
    getErrorCorrection(cw, ecl) {
        ...
        const ecw = Array(eclsize).fill(0);
...
PDF417.DEFAULT_ROWHEIGHT = 3; // PDF417 spec typical is 3X to 5X
PDF417.DEFAULT_QUIETH = 2;
PDF417.DEFAULT_QUIETV = 2;
PDF417.DEFAULT_ASPECT_RATIO = 2;
PDF417.DEFAULT_ECL = -1; // Automatic
PDF417.START_PATTERN = "11111111010101000";
PDF417.STOP_PATTERN = "111111101000101001";
PDF417.TEXT_SUBMODES = [
    [
...
PDF417.CLUSTERS = pdf417_clusters_1.CLUSTERS;
PDF417.RS_FACTORS = [
